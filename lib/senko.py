...
class Senko:
    ...
    def _get_file(self, url):
        ...
        if code == 200:
            ...
            print("[Senko] Problem accessing github {}".format(url))
            return None
    ...
    def update(self):
        ...
        """
        print("[Senko] Checking for updates...")
        changes = self._check_all()
        print("[Senko] No update needed") if len(changes) == 0 else print("[Senko] Updating files...")

        for file in changes:
            with open(file, "w") as local_file:
                print("[Senko] Write {}".format(file))
